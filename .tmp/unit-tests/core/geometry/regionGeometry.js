...
exports.getRegionDependencyIds = getRegionDependencyIds;
exports.getRegionBoundaryPath = getRegionBoundaryPath;
exports.getRegionArea = getRegionArea;
exports.regionContainsPoint = regionContainsPoint;
const math_1 = require("./math");
const derivedGeometry_1 = require("./derivedGeometry");
const BoundaryFillEngine_1 = require("./regions/BoundaryFillEngine");
...
function edgeObject(edge, objects) {
    return objects[edge.objectId] ?? null;
}
function edgeStartEnd(edge, objects) {
    if (edge.startParameter !== undefined && edge.endParameter !== undefined) {
        const primitive = (0, BoundaryFillEngine_1.collectBoundaryPrimitives)(objects).find((candidate) => candidate.id === edge.sourcePrimitiveId ||
    ...
    const start = edge.startPointId ? (0, derivedGeometry_1.getPointObject)(objects, edge.startPointId) : null;
    const end = edge.endPointId ? (0, derivedGeometry_1.getPointObject)(objects, edge.endPointId) : null;
    if (start && end) {
    ...
    if (object?.type === "segment" || object?.type === "vector") {
        const objectStart = (0, derivedGeometry_1.getPointObject)(objects, object.startPointId);
        const objectEnd = (0, derivedGeometry_1.getPointObject)(objects, object.endPointId);
...
function arcCommand(center, radius, startDegrees, endDegrees, direction) {
...
function edgePath(edge, objects, first) {
    const object = edgeObject(edge, objects);
    ...
    if (edge.edgeKind === "circle" && object?.type === "circle") {
        const circle = (0, derivedGeometry_1.getCircleGeometry)(object, objects);
        if (!circle) {
            return null;
        }
    ...
    if (edge.edgeKind === "arc") {
        if (object?.type === "circle") {
            ...
            const path = arcCommand(circle.center, circle.radius, edge.startParameter, edge.endParameter, edge.direction);
        ...
        if (object?.type === "arc") {
            ...
            const path = arcCommand(geometry.center, geometry.radius, startAngle, endAngle, edge.direction);
            return first ? path : path.replace(/^M\s+[-0-9.]+\s+[-0-9.]+\s+/, "");
        }
    }
    return null;
}
function getRegionDependencyIds(object) {
    if (object.regionKind === "boundary" && object.loops) {
        const ids = object.loops.flatMap((loop) => loop.edges.flatMap((edge) => [
            edge.objectId,
...
function getRegionBoundaryPath(object, objects) {
...
function getRegionArea(object, objects) {
    const boundary = getRegionBoundaryPath(object, objects);
    if (!boundary) {
        return null;
    }
    if (boundary.kind === "polygon") {
        return Math.abs((0, math_1.polygonArea)(boundary.points));
    }
    return null;
}
function regionContainsPoint(object, point, objects) {
    const boundary = getRegionBoundaryPath(object, objects);
    if (!boundary) {
        return false;
    }
    if (boundary.kind === "polygon") {
        return (0, derivedGeometry_1.isPointInPolygon)(point, boundary.points);
    }
    return false;
}
