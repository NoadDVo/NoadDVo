...
exports.buildGeometryFromTikzAst = buildGeometryFromTikzAst;
...
function nextId(state, prefix, hint) {
...
function createPoint(state, name, point) {
...
function createGeneratedPoint(state, prefix, point) {
    const name = `${prefix}${state.sequence + 1}`;
    return createPoint(state, name, point);
}
function pointForPathPoint(state, pathPoint) {
...
function existingPointAt(state, point) {
    return Array.from(state.objects.values()).find((object) => object.type === "point" && (0, geometry_1.pointsAlmostEqual)(object, point, 1e-6)) ?? null;
}
function namedPoints(state) {
    return Array.from(state.objects.values()).filter((object) => object.type === "point" && Boolean(object.name));
}
...
function pointLiesOnLine(point, lineA, lineB) {
    return Math.abs(crossProduct(lineA, lineB, point)) <= 1e-5;
}
function pointLiesOnRay(point, start, end) {
    return pointLiesOnLine(point, start, end) && dotProduct(start, end, point) >= -1e-6;
}
function parseOptions(options, state) {
    const style = { ...geometry_1.DEFAULT_GEOMETRY_STYLE };
...
function readColorDefinition(command, state) {
    const match = compact(command.argumentText).match(/^\{\s*([^}]+?)\s*\}\s*\{\s*HTML\s*\}\s*\{\s*([0-9a-fA-F]{6})\s*\}$/i);
    if (!match?.[1] || !match[2]) {
        return;
    }
    state.colorDefinitions.set(match[1].trim(), `#${match[2].toUpperCase()}`);
}
function recoverCoordinate(command, state) {
    const pattern = new RegExp(`^\\(\\s*([A-Za-z][A-Za-z0-9]*)\\s*\\)\\s*at\\s*\\(\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*\\)$`);
...
function readPathPoints(argumentText) {
    const points = [];
    const pattern = new RegExp(`\\(\\s*([A-Za-z][A-Za-z0-9]*)\\s*\\)|\\(\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*\\)`, "g");
...
function recoverClippedLinearPath(command, state, pathPoints) {
    ...
    const rayStart = points.find((point) => (0, geometry_1.pointsAlmostEqual)(point, start, 1e-6));
    if (rayStart) {
        const through = points.find((point) => point.id !== rayStart.id &&
            !(0, geometry_1.pointsAlmostEqual)(point, rayStart, 1e-6) &&
    ...
    for (let firstIndex = 0; firstIndex < points.length; firstIndex += 1) {
        ...
        for (let secondIndex = firstIndex + 1; secondIndex < points.length; secondIndex += 1) {
            ...
            if (!second || (0, geometry_1.pointsAlmostEqual)(first, second, 1e-6) || !pointLiesOnLine(second, start, end)) {
                continue;
            }
            const style = parseOptions(command.options, state);
...
function recoverLinearPath(command, state) {
    ...
    if (points.length >= 3 && pathUsesCycle(command.argumentText)) {
        ...
        const object = command.name === "draw"
            ? {
                ...baseObject(state, id, style),
                closed: true,
    ...
    if (points.length === 2) {
        ...
        const style = parseOptions(command.options, state);
        const id = nextId(state, optionHasArrow(command.options) ? "vector" : "segment", `${start.name ?? start.id}-${end.name ?? end.id}`);
        const object = optionHasArrow(command.options)
            ? {
                ...baseObject(state, id, style),
                endPointId: end.id,
...
function recoverCircle(command, state) {
    if (command.name !== "draw" && command.name !== "filldraw") {
        return false;
    }
    const pattern = new RegExp(`^\\(\\s*([A-Za-z][A-Za-z0-9]*)\\s*\\)\\s*circle\\s*\\(\\s*(${numberPattern})`);
    const literalPattern = new RegExp(`^\\(\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*\\)\\s*circle\\s*\\(\\s*(${numberPattern})`);
    ...
    if (!namedCenter && literalPoint) {
        const circlePoints = namedPoints(state).filter((point) => Math.abs((0, geometry_1.distance)(point, literalPoint) - radius) <= 1e-5);
    ...
    const circlePoints = namedPoints(state).filter((point) => Math.abs((0, geometry_1.distance)(point, center) - radius) <= 1e-5);
...
function recoverArc(command, state) {
    const startPattern = new RegExp(`^\\(\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*\\)\\s*arc\\s*\\[([^\\]]+)\\]`);
    const text = compact(command.argumentText);
    ...
    const valueFor = (key) => {
        const entry = arcOptions.find((option) => new RegExp(`^${key}\\s*=`).test(option));
...
function labelPositionFromOptions(options) {
...
function recoverMeasurementNode(command, state, point, content) {
    ...
    const objects = Object.fromEntries(state.objects.entries());
    const createProbe = (targetObjectId, measurementType) => ({
        createdAt: 0,
    ...
    const target = Object.values(objects).find((object) => measurementTypes.some((measurementType) => {
        if (!(0, geometry_1.isMeasurementTypeSupported)(object, measurementType)) {
            return false;
        }
        return normalizedMeasurementContent((0, geometry_1.formatMeasurementValue)(createProbe(object.id, measurementType), objects)) === normalizedContent;
    }));
    if (!target) {
        return false;
    }
    const measurementType = measurementTypes.find((candidate) => (0, geometry_1.isMeasurementTypeSupported)(target, candidate) &&
        normalizedMeasurementContent((0, geometry_1.formatMeasurementValue)(createProbe(target.id, candidate), objects)) === normalizedContent);
    if (!measurementType) {
        return false;
    }
...
function recoverNode(command, state) {
    const text = compact(command.argumentText);
    const match = text.match(/\bat\s*\(\s*([A-Za-z][A-Za-z0-9]*)\s*\)\s*\{\s*(.*?)\s*\}$/);
    const literalMatch = text.match(new RegExp(`\\bat\\s*\\(\\s*(${numberPattern})\\s*,\\s*(${numberPattern})\\s*\\)\\s*\\{\\s*(.*?)\\s*\\}$`));
    if (match?.[1]) {
...
function recoverPic(command, state) {
...
function recoverPathCommand(command, state) {
...
function buildGeometryFromTikzAst(ast) {
    const state = {
    ...
    const normalized = (0, geometry_1.normalizeDependencyMetadata)(Object.fromEntries(state.objects.entries()));
    return {
        issues: state.issues,
