...
function tokenText(tokens: readonly TikzToken[]): string {
  return tokens.map((token) => token.value).join(" ");
}

function optionText(tokens: readonly TikzToken[]): string {
  return tokenText(tokens).replace(/\s*,\s*/g, ",").trim();
}

function splitOptions(tokens: readonly TikzToken[]): readonly string[] {
  const options: string[] = [];
  let current: TikzToken[] = [];
  let braceDepth = 0;
  let parenDepth = 0;
...
function validateBalance(tokens: readonly TikzToken[], command: TikzToken): TikzParseIssue[] {
  const issues: TikzParseIssue[] = [];
  const stack: TikzToken[] = [];
...
function consumeBracketGroup(tokens: readonly TikzToken[], start: number): number {
  let depth = 0;
...
function buildCommand(command: TikzToken, body: readonly TikzToken[]): TikzCommandNode {
  let bodyStart = 0;
...
function commandNeedsSemicolon(commandName: string): boolean {
  return ["coordinate", "draw", "fill", "filldraw", "node", "pic"].includes(commandName);
}

export function parseTikzAst(tokens: readonly TikzToken[]): AstParseResult {
  const commands: TikzCommandNode[] = [];
  const issues: TikzParseIssue[] = [];
  let index = 0;
